...
(function (document){
	...
	//Menu
	let menuActivator = document.getElementById('menu_activator');
	let headerMenu = document.getElementById('header_menu');
	if (menuActivator && headerMenu) {
		menuActivator.addEventListener('click', () => {
			headerMenu.style.display = (headerMenu.style.display === '' || headerMenu.style.display === 'none') ? 
				'block' : 'none';
	  });
	}
